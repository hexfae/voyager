...
pub enum Error {
    /// POST and PUT: The input data was invalid.
    ///
    /// Either it had too many separators, not enough separators,
    /// or was otherwise incorrect. Input data should contain 11
    /// separators ('|') for POST and 12 separators for PUT.
    ...
    /// POST and PUT: The name was invalid.
    ///
    /// Either it was invalid Base64, was too small (0), was
    /// too big (>[`MAX_NAME_LEN`]), or was somehow decoded
    /// into invalid UTF-8.
    #[error("invalid name: {0}")]
    InvalidName(StringError),
    /// POST and PUT: The description was invalid.
    ///
    /// Either it was invalid Base64, was too big (>[`MAX_DESCRIPTION_LEN`]),
    /// or was somehow decoded into invalid UTF-8.
    #[error("invalid description: {0}")]
    InvalidDescription(StringError),
    /// POST and PUT: The chosen music was invalid.
    ///
    /// Either it was invalid Base64, or was somehow decoded into
    /// invalid UTF-8.
    #[error("invalid music: {0}")]
    InvalidMusic(StringError),
    /// POST and PUT: The chosen music was not found.
    ///
    /// The song was not in the configured list of allowed song names.
    #[error("invalid music: not a valid song")]
    NotASong,
    /// POST and PUT: The author was invalid.
    ///
    /// Either it was invalid Base64, was too small (0), was too big
    /// (>[`MAX_AUTHOR_LEN`]), or was somehow decoded into invalid UTF-8.
    #[error("invalid author: {0}")]
    InvalidAuthor(StringError),
    /// POST and PUT: The author brand was invalid.
    ///
    /// Either it was not a number, or was too big ([`BRAND_36_BITS`]).
    #[error("invalid brand: {0}")]
    InvalidBrand(NumberError),
    /// POST and PUT: The level burdens were invalid.
    ///
    /// Either it was not a number, or was too big ([`BURDENS_4_BITS`]).
...
pub enum NumberError {
    ...
    /// POST and PUT. Number was too small.
